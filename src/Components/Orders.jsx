...
// 1. Added the ID Mapping Object (Same as your Checkout component)
const IMAGE_ID_MAPPING = { 
  ...
  // ... (Add any other IDs you use in your DB here)
};
...
function Orders() {
  ...
  const fetchAllOrders = async () => {
    ...
    try {
      ...
      Object.entries(allUsersData || {}).forEach(([userId, userOrders]) => {
        ...
        Object.entries(userOrders).forEach(([orderId, orderData]) => {
          ...
          if (!orderData.delivered) {
            let products = [];
            if (orderData.items) {
              products = Object.values(orderData.items);
            } else {
              const metaFields = ["address", "phno", "username", "orderTimestamp", "orderTotal", "ordered", "delivered", "notes"];
              products = Object.keys(orderData)
                .filter((k) => !metaFields.includes(k) && typeof orderData[k] === "object")
                .map((k) => orderData[k]);
            }
      ...
      allOrders.sort((a, b) => (b.orderTimestamp || 0) - (a.orderTimestamp || 0));
      setOrders(allOrders);
    } catch (err) {
  ...
  // ✅ Modified Image Resolver to handle IDs
  const getImageUrl = (imageValue) => {
    ...
    // 1. Check if it's a Numeric ID from the mapping
    if (IMAGE_ID_MAPPING[val]) {
      return `/${IMAGE_ID_MAPPING[val]}`;
    }

    // 2. Check for direct URL
    if (val.startsWith("http")) {
      return val;
    }

    // 3. Check for direct filename string
  ...
  return (
    <div className="orders-containera">
      ...
      {loading ? (
        <div className="loading-containera"><div className="loading-spinnera"></div></div>
      ) : error ? (
        <div className="error-containera"><p>{error}</p></div>
      ) : orders.length === 0 ? (
        <div className="empty-ordersa"><h2>No Pending Orders</h2></div>
      ) : (
        <div className="orders-lista">
          {orders.map((order) => (
            <div key={order.orderId} className="order-carda" onClick={() => handleOrderClick(order)}>
              <div className="order-headera">
                <div className="order-infoa">
                  <span className="order-ida">Order #{order.orderId.substring(0, 8)}</span>
                </div>
                <div className="order-statusa">
                   <span className={`status-badge ${order.ordered ? 'pending' : 'delivered'}`}>
                     {order.ordered ? "Processing" : "Pending"}
                   </span>
                </div>
              </div>
                ...
                {order.products.map((item, index) => (
                  <div key={index} className="product-itema">
                    <div className="product-imagea">
                      <img
                        src={getImageUrl(item?.productimage || item?.productimageurl)}
                        alt="Product"
                        onError={(e) => { e.target.src = "/unknowenprofile.png"; }}
                      />
                    </div>
                    <div className="product-detailsa">
                      <h3>{item?.productname || "Unknown Item"}</h3>
                      <div className="product-metaa">
                        <span>Qty: {item?.qty || 1}</span>
                        <span className="product-pricea">₹{item?.productamt || 0}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="order-footera">
                <div className="shipping-infoa">
                  <p><strong>To:</strong> {order.username} | {order.phno}</p>
