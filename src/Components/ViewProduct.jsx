...
// We only import the specific functions we need from the SDKs
// ADDED Firebase app/auth/db/storage imports
...
/*
  REMOVED the import for './firebase.js' to fix the build error.
  This component will now initialize Firebase itself, checking if an
  app already exists to prevent runtime errors.
*/
// REMOVED: import { database, storage } from './firebase.js';
...
function ViewProduct() { // Renamed from ViewProduct to BookAdmin
  // --- States for the View List ---
  const [books, setBooks] = useState([]);
  const [loadingList, setLoadingList] = useState(true);
  
  // --- ADDED Firebase Instances State ---
  // We'll store the initialized db and storage in state.
  ...
  // --- 0. ADDED FIREBASE INITIALIZATION ---
  // This useEffect hook initializes Firebase when the component mounts.
  useEffect(() => {
    try {
      // Use global config variables provided in the environment
      const firebaseConfigStr = typeof window.__firebase_config !== 'undefined' ? window.__firebase_config : '{}';
      const firebaseConfig = JSON.parse(firebaseConfigStr);

      if (!firebaseConfig.apiKey || !firebaseConfig.databaseURL) {
          console.warn("Firebase config is missing or incomplete! Using default/demo values.");
          // We can still proceed, and a failure will be caught by the DB listeners.
      }
      
      // Prevent "duplicate app" error if it's already initialized by another component
      ...
      const signIn = async () => {
        try {
          ...
          // Now we're ready to make DB calls
          setIsAuthReady(true);
        } catch (authError) {
          console.error("Firebase sign-in error:", authError);
          showNotification("Could not authenticate with Firebase.", "error");
          setLoadingList(false); // Stop loading if auth fails
        }
      };
      ...
      showNotification("Could not connect to Firebase.", "error");
      setLoadingList(false); // Stop loading if init fails
    }
  }, []); // Runs once on mount
  ...
  useEffect(() => {
    // --- ADDED Auth/DB Check ---
    // Wait until Firebase is initialized and user is authenticated
    if (!isAuthReady || !db) {
      if (isAuthReady && !db) {
         // This case might happen if DB init failed
         showNotification("Database not available.", "error");
         setLoadingList(false);
      }
      // If we're not ready, just wait. Don't set loading to false.
      return;
    }
    ...
    try {
      // *** DB NODE: "books" ***
      // Use the 'db' instance from state
      ...
      const unsubscribe = onValue(
        ...
        (error) => {
          ...
          showNotification('Failed to load book items.', 'error');
          setLoadingList(false);
        }
      );
      ...
      return () => unsubscribe(); // Cleanup listener on unmount
        ...
        // This catch block will handle errors
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { title, summary, yearPublished, author, edition, price, discount, imageFile } = formState;

    if (!title || !author || !yearPublished || !summary || !price || (!imageFile && !editingItemKey)) {
    ...
    try {
       // Check if Firebase is available (using state instances)
       if (!db || !storage) {
      ...
      if (editingItemKey) {
        ...
        if (imageFile) {
          // *** NEW STORAGE PATH: "book-covers/" ***
          // Use the 'storage' instance from state
          const newImageRef = storageRef(storage, `book-covers/${editingItemKey}_${imageFile.name}`);
        ...
        // *** NEW DB NODE: "books" ***
        // Use the 'db' instance from state
        await update(dbRef(db, `books/${editingItemKey}`), {
          title: title,
          author: author,
        ...
        // Use the 'db' instance from state
        const booksRef = dbRef(db, 'books');
        const lastBookQuery = query(booksRef, orderByKey(), limitToLast(1));
        const snapshot = await get(lastBookQuery);

        // A new, simpler key-gen (Firebase push keys are better, but for this demo):
        let nextKey = 1000000001;
        if (snapshot.exists()) {
        ...
        // *** NEW STORAGE PATH: "book-covers/" ***
        // Use the 'storage' instance from state
        const imageRef = storageRef(storage, `book-covers/${nextKey}_${imageFile.name}`);
        await uploadBytes(imageRef, imageFile);
        const imageUrl = await getDownloadURL(imageRef);

        // *** NEW DB NODE: "books" ***
        // Use the 'db' instance from state
        await set(dbRef(db, `books/${nextKey}`), {
          title: title,
          author: author,
          yearPublished: parseInt(yearPublished, 10),
          edition: edition,
    ...
    try {
      ...
      if (item.imageUrl) {
        try {
          ...
          // *** NEW STORAGE PATH: "book-covers/" ***
          if (imagePath.startsWith('book-covers/')) {
              // Use the 'storage' instance from state
              const imageRef = storageRef(storage, imagePath);
              await deleteObject(imageRef);
              console.log("Image deleted successfully");
          } else {
              console.warn("Image path not in expected 'book-covers/' folder. Skipping delete.");
          }
        } catch (imgErr) {
      ...
      // *** NEW DB NODE: "books" ***
      // Use the 'db' instance from state
      await remove(dbRef(db, `books/${item.id}`));
  ...
  const renderList = () => {
    if (loadingList) {
      return <div className="loading-text">Loading books...</div>;
    }
    if (books.length === 0) {
      return <div className="empty-text">No book items found.</div>;
    }
          ...
          return (
            <div className="product-card" key={item.id}>
              ...
              <div className="product-card-body">
                <h5 className="product-card-title">{item.title}</h5>
                <h6 className="product-card-brand">
                  By {item.author} ({item.yearPublished})
                </h6>
                <p className="product-card-desc">
                  {item.edition && `Edition: ${item.edition} - `}{item.summary}
                </p>
        ...
        <div className="modal-backdrop" onClick={hideDeleteModal}>
          <div className="modal-content modal-content-delete" onClick={(e) => e.stopPropagation()}>
             ...
             <p className="modal-body-delete">
                Are you sure you want to delete "<strong>{deleteModal.item?.title || 'this item'}</strong>"?
      ...
      <div className="main-content-wrapper">
        ...
        <div className="card-container">
          <h2 className="card-title">
            {editingItemKey ? 'Edit Book Item' : 'Add Book Item'}
          </h2>
          <form id="book-form" ref={formRef} onSubmit={handleSubmit} className="form-space-y">
            ...
            <div className="form-grid-cols-2">
              ...
              <input
                className="form-input"
                type="text"
                placeholder="Edition (e.g., 2nd, Hardcover)"
                name="edition"
            ...
            <div>
              <label htmlFor="formFile" className="form-label">
                {editingItemKey ? 'Upload new cover (optional)' : 'Book Cover Image'}
              </label>
              <input
            ...
            <div className="form-actions" style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              >
                ...
                ) : (editingItemKey ? 'Update Book' : 'Add Book')}
              </button>
              {editingItemKey && (
                <button
                  type="button"
...
export default ViewProduct; // Renamed from ViewProduct to BookAdmin
