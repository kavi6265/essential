...
function OrderedProductpreview() {
  ...
  const fetchOrderDetails = (userId, orderId) => {
    ...
    onValue(orderRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const ordered = orderData.ordered; // Using 'ordered' (was 'odered' in previous file)
  ...
  const handleProductClick = (product) => {
    const formattedProduct = {
      ...
      // ==========================================================
      //  FIX 1: Check for productimageurl first, then fallback
      // ==========================================================
      img: product.productimageurl ? product.productimageurl : getImageUrl(product.productimage),
          ...
          <div className="order-summary-card">
            <div className="order-header-section">
              <div className="order-id-section">
                <h2>Order #{order.orderId.substring(order.orderId.length - 8)}</h2>
              ...
              {order.products.map((product, index) => (
                <div key={index} className="order-product-item" onClick={() => handleProductClick(product)}>
                  <div className="product-image">
                    <img
                      // ==========================================================
                      //  FIX 2: Check for productimageurl first, then fallback
                      // ==========================================================
                      src={product.productimageurl ? product.productimageurl : getImageUrl(product.productimage)}
